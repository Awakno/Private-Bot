...
class LogEvents(commands.Cog):
    ...
    async def on_guild_channel_create(self, channel):
        ...
        if log_channel:
            ...
            embed.add_field(name=f"Salon créé ({channel_type})", value=f"Nom : {channel.name}\nID : {channel.id}")
            async for entry in channel.guild.audit_logs(limit=1, action=discord.AuditLogAction.channel_create):
                author = entry.user
            embed.add_field(name="Créé par", value=f"{author.mention}")
            embed.set_author(name=f"Salon créé ({channel_type})", icon_url=channel.guild.icon.url)
    ...
    async def on_member_kick(self, guild, member):
        ...
        if log_channel:
            ...
            embed.add_field(name=f"{action_type} d'un membre", value=f"Utilisateur : {member.mention}\nServeur : {guild.name}")
    ...
    async def on_member_ban(self, guild, user):
        ...
        if log_channel:
            embed = discord.Embed(timestamp=datetime.now())
            embed.add_field(name="Membre banni", value=f"Utilisateur : {user.mention}\nServeur : {guild.name}")
            ...
            async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if entry.target == user:
                    author = entry.user
            embed.add_field(name="Par Bannissement", value=f"{author.mention}")
