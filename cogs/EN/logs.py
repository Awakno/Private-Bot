...
class LogEvents(commands.Cog):
    def __init__(self, bot):
        ...
        config = json.load(f)
        self.log_channel_id = int(config['logs_channel'])   # Replace this with the ID of the logs channel
    ...
    async def on_member_update(self, before, after):
        ...
        if log_channel:
            if before.nick != after.nick:
                embed = discord.Embed(timestamp=datetime.now())
                embed.add_field(name="Nickname change", value=f">>> {after.mention}\nNew nickname: {after.nick}")
    ...
    async def on_guild_channel_create(self, channel):
        ...
        if log_channel:
            ...
            # Get the author of the channel creation from the audit log
